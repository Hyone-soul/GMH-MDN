...
echo "### modelarts_flag ${modelarts_flag}"
...
wait
#nohup python3 ./predict_3dpose_mdm.py \
#--test=${test} \
#--data_dir=${data_dir} \
#--train_dir=${train_dir} \
#--load_dir=${load_dir} \
...
#输出性能FPS，需要模型审视修改
StepTime=`grep "Working on epoch" ${print_log} | tail -n 10|awk '{print $11}' | awk '{sum+=$1} END {print sum/NR/1000}'`
#打印，不需要修改
FPS=`awk 'BEGIN{printf("%.2f\n", '${batch_size}' /'${StepTime}')}'`

#输出训练精度,需要模型审视修改
train_accuracy=`grep "predict_3dpose_mdm.py:309" ${print_log} | awk 'END {print $6}'`

# 提取所有loss打印信息
grep "Train loss avg:" ${print_log} | awk '{print $4}' > ./test/output/${ASCEND_DEVICE_ID}/my_output_loss.txt
